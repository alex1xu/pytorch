...
def linspace(
    ...
    *,
    ...
    # Match eager: integer linspace truncates the endpoints toward zero before
    # interpolating (see linspace_kernel), so the float endpoints don't round
    # differently than eager does. The truncated value is kept in wide precision
    # so the step subtraction below can't overflow the (narrow) output dtype.
    # NumPy doesn't have this cast.
    if utils.is_integer_dtype(dtype):
